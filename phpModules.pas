...
type
  ...
  private
    ...
    //Added by Luis E.
    FDeferredLoading: Boolean;
    FDeferredList: TThreadList;
    FClassesLoaded: Boolean;
  ...
  public
    ...
    procedure   DeferClassRegistry(ClassComponent: TComponent);
    ...
    property    DeferredLoading: Boolean read FDeferredLoading write FDeferredLoading;
    property    ClassesRegistered: Boolean read FClassesLoaded write FClassesLoaded;  
  end;


function get_module : Pzend_module_entry; cdecl;
procedure set_deferred_class_loading(value: boolean);
function get_deferred_class_loading: Boolean;
...
var
  ...
  //Added by Luis E.
  defer_class_loading: boolean = false;

implementation
  uses phpClass, Dialogs;
  ...
  ts_allocate_id(@app_globals_id, sizeof(pointer), nil, nil);
     if Assigned(Application) then
      begin
       ...
       if Application.DeferredLoading then
         Application.RegisterClasses;
...
procedure set_deferred_class_loading(value: boolean);
begin
  defer_class_loading:= value;
end;

//added by luise
function get_deferred_class_loading: Boolean;
begin
  result:= defer_class_loading;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      Application.DeferredLoading:= defer_class_loading;
      ...
      {$ELSE}
        {$IFDEF PHP510}
          //originally it had the commented code. Added by luis E
          ModuleEntry.Name := StrNew(PChar(Extension.ModuleName)); //estrndup(PChar(Extension.ModuleName), length(extension.ModuleName));
          ModuleEntry.version := StrNew(PChar(Extension.Version));//estrndup(PChar(Extension.Version), length(Extension.Version));
...
begin
  ...
  FDeferredList:= TThreadList.Create;
  FDeferredList.Duplicates:= dupError;
...
procedure TPHPApplication.DeferClassRegistry(ClassComponent: TComponent);
begin
  try
  if FDeferredLoading then begin
    FDeferredList.Add(ClassComponent);
  end;
  except
   on e: EListError do //nothing to do
  end;
end;

procedure TPHPApplication.RegisterClasses();
var
  i: Integer;
  List: TList;
begin
  if ClassesRegistered then Exit;
  List:=FDeferredList.LockList;

  try
    for i := 0 to List.Count-1 do begin
      if (List.Items[i] <> nil) then begin
        TPHPClass(List.Items[i]).ClassRegister(Application.ModuleNumber);
      end;
    end;
    ClassesRegistered := True;
  finally
    FDeferredList.UnlockList;
  end;
end;
...
exports
  get_module,
  set_deferred_class_loading,
  get_deferred_class_loading;
end.
