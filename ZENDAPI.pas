...
//                            char *message, long code TSRMLS_DC);
  ...
  zend_throw_exception: function (exception_ce: pzend_class_entry; message: pchar; code: ulong; TSRMLS_DC : pointer): pzval;

//  zval * zend_throw_exception_ex(zend_class_entry *exception_ce,
//                            long code TSRMLS_DC, char *format, ...);

  zend_throw_exception_ex: function (exception_ce: pzend_class_entry; code: ulong; TSRMLS_DC : pointer; format: pchar; const args: array of const): pzval;

//zval * zend_throw_error_exception(zend_class_entry *exception_ce,
//                            char *message, long code, int severity TSRMLS_DC);

  zend_throw_error_exception: function (exception_ce: pzend_class_entry; message: pchar; code: ulong; serverity: integer; TSRMLS_DC : pointer): pzval;

//void zend_throw_exception_object(zval *exception_obj TSRMLS_DC);

  zend_throw_exception_object: procedure (exception_obj: pzval; TSRMLS_DC : pointer);
...
{$ENDIF}
  ...
  //Added by LuisE
  zend_throw_exception:= GetProcAddress(PHPLib, 'zend_throw_exception');

  zend_throw_exception_ex:= GetProcAddress(PHPLib, 'zend_throw_exception_ex');

  zend_throw_error_exception:= GetProcAddress(PHPLib, 'zend_throw_error_exception');

  zend_throw_exception_object:= GetProcAddress(PHPLib, 'zend_throw_exception_object');
...
begin
  ...
  {$IFDEF PHP5}
    ...
    {$ELSE}
      {$IFDEF PHP512}
      //Added By Luis E. I made the same thing for both PHP20 and PHP12
      class_container.name := PChar(class_name);
      // comment Added by luisE
