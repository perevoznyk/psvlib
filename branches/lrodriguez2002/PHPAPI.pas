...
function GetVariantArray(val: pzval) : Variant;
...
var
 data: ^ppzval;
 elem_count, i: Integer;
begin
  new(data);
  elem_count:= zend_hash_num_elements(val.value.ht);
  Result:=VarArrayCreate([0, elem_count-1], varVariant);
  for i:= 0 to elem_count-1 do begin
    zend_hash_index_find(val.value.ht, i, data);
    Result[i]:= zval2variant(data^^^);
  end;
end;
...
begin
  case Value._type of
   ...
   IS_ARRAY   : Result := GetVariantArray(@value);
   IS_OBJECT  : Result:=  Integer(@Value);
   else
      Result := NULL;
  end;
end;
...
begin
  if VarIsEmpty(value) then
   ...
   if VarIsArray (value) then begin
        ...
        for cnt := LBound to HBound do
         begin
           ztemp:= MAKE_STD_ZVAL;
           variant2zval(value[cnt], ztemp);
           add_next_index_zval(z, ztemp);
         end;
        Exit;
   end;
