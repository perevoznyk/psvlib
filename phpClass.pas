...
uses
  Windows, SysUtils, Classes, PHPCommon, Contnrs,
...
type
  ...
  TPHPPropertySetter = procedure(Value: Variant) of object;
  TPHPPropertyGetter = function: Variant of object;
  ...
  //Property Types
  TPropertyDataType = (ptString, ptObject);

  //Property
  //Nota cuando se añada una propiedad debe modificarse
  //el procedimiento Assign
  TClassProperty = class(TCollectionItem)
  private
    FName  : string;
    FValue : string;
    FZendVariable : TZendVariable;
    FPropertyGetter: TPHPPropertyGetter;
    FPropertySetter: TPHPPropertySetter;
    FReadOnly: Boolean;
    FPropertyType: TPropertyDataType;
  ...
  published
    ...
    property PropertyValue : string read GetAsString write SetAsString;
    property OnGetValue: TPHPPropertyGetter read FPropertyGetter write FPropertyGetter;
    property OnSetValue: TPHPPropertySetter read FPropertySetter write FPropertySetter;
    property ReadOnly: Boolean read FReadOnly write FReadOnly;
    property DataType: TPropertyDataType read FPropertyType write FPropertyType;
  ...
  //pass the object recently created...
  TInstanceCreateFunc = procedure(obj : TPHPClassInstance; Params: Variant) of object;
  TInstanceDestroyFunc = procedure (obj: TPHPClassInstance) of object;
  ...
  private
    ...
    FOnInstanceCreate: TInstanceCreateFunc;
    FOnInstanceDestroy: TInstanceDestroyFunc;
  ...
  public
    ...
    function ProduceInstance(AValue : pzval; ht: integer): TPHPClassInstance;  virtual;
  published
    ...
    property OnInstanceCreate: TInstanceCreateFunc read FOnInstanceCreate
           write FOnInstanceCreate;
    property OnDestroyInstance: TInstanceDestroyFunc read FOnInstanceDestroy
           write FOnInstanceDestroy;
  end;


  //Added by Luis E:  esta clase se decaró para mantener una lista de
  // las instancias de los objectos y una lista de propiedades que pueden ser
  //objectos de delphi.

  TObjectPropertyList = class
    public
    _instance: TPHPClassInstance;
    _propertyList: TStringList;
    constructor Create( _instance: TPHPClassInstance);
  end;
  ...
  private
    FProperties : TClassProperties;
    FProducer   : TPHPClass;
    ObjectInternalProps: TObjectPropertyList;
    _php_obj: pzval;
    procedure SetProperties(const Value: TClassProperties);
  protected
  ...
  public
    constructor Create(AOwner : TComponent); override;
    destructor  Destroy; override;
    property ObjectPropList: TObjectPropertyList read ObjectInternalProps write ObjectInternalProps;
    property    Properties : TClassProperties read FProperties write SetProperties;
    property _php_object : pzval read _php_obj write _php_obj;
  end;
...
//Added by luis E
function get_object_property(PropName: string; _instance: Pointer): Pointer;

function get_instance_by_property_val(PropName: string; _prop_value: Pointer): TPHPClassInstance;

function set_object_internal_property(PropName: string; prop_value : Pointer; _instance: Pointer): Boolean;

//Added by luis e
//Setting and restoring the contexts for the use of the objects internal properties.
// Returns the current instance if no instance context available returns nil.
function _current_instance: TPHPClassInstance;

//setups the
procedure set_current_instance(_instance: TPHPClassInstance);
function unset_current_instance(): TPHPClassInstance;

//set instace property
function get_instance_from_zval(_object : pzval; TSRMLS_DC : pointer): TPHPClassInstance;

//obtain the active simbol table
function get_active_simbol_table(TSRMLS_DC : pointer) : PHashTable;

function object_result(obj_pointer: Pointer): string;

//set the property for the current object
procedure set_instance_property(Obj: TPHPClassInstance; PropName: string; PropValue: Variant);

//throw exceptions in php


var
  _objects_list: TThreadList;
  _context_stack: TStack;
...
function object_result(obj_pointer: Pointer): string;
begin
  Result:= IntToStr(Integer(obj_pointer));
end;

//Added by luis E
function get_active_simbol_table(TSRMLS_DC : pointer) : PHashTable;
begin
  Result := @GetExecutorGlobals(TSRMLS_DC).active_symbol_table;
end;

//Added by luis E.
function _current_instance: TPHPClassInstance;
begin
  if _context_stack.Count <> 0 then begin
    Result:= _context_stack.Peek;
  end
  else Result:= nil;
end;
...
procedure set_current_instance(_instance: TPHPClassInstance);
begin
  Assert(_instance <> nil, 'No se puede insertar una instancia con valor nil');
  _context_stack.Push(_instance);
end;

function unset_current_instance(): TPHPClassInstance;
begin
  if _context_stack.Count <> 0 then begin
    Result:= _context_stack.Pop;
  end
  else Result:= nil;
end;
...
function set_object_internal_property(PropName: string; prop_value : Pointer; _instance: Pointer): Boolean;
var
  local_instance: Pointer;
  local_object_list: TList;
  index: Integer;
  prop_index: Integer;
  _instance_props: TStringList;
begin
  Result:= False;
  if _instance = nil then  begin //use the current intance
    local_instance:= _current_instance;
  end
  else local_instance:= _instance;

  if local_instance <> nil then begin
    local_object_list:= _objects_list.LockList;
    try
      index:= local_object_list.IndexOf(local_instance);

      if index = -1 then begin
        _objects_list.Add(local_instance);
      end;
      if TPHPClassInstance(local_instance).ObjectInternalProps = nil then
         TPHPClassInstance(local_instance).ObjectInternalProps := TObjectPropertyList.Create(local_instance);

      _instance_props:= TPHPClassInstance(local_instance).ObjectInternalProps._propertyList;
      if (_instance_props = nil) then begin
        TPHPClassInstance(local_instance).ObjectInternalProps._propertyList := TStringList.Create;
        _instance_props:= TPHPClassInstance(local_instance).ObjectInternalProps._propertyList;
      end;

      prop_index := _instance_props.IndexOf(PropName);
      if prop_index = -1 then begin
         _instance_props.AddObject(PropName, TObject(prop_value));
      end
      else begin
         _instance_props.Objects[prop_index]:= TObject(prop_value);
      end;
    finally
       _objects_list.UnlockList;
    end;
...
//Added by luis E.
//Función para establecer el valor de una propiedad dada la intancia no el objeto de php
//la propiedad es del tipo de las que están disponibles en PHP.
procedure set_instance_property(Obj: TPHPClassInstance; PropName: string; PropValue: Variant);
var
...
//Added by luis e.
function get_instance_property(Obj: TPHPClassInstance; PropName: string; PropValue: Variant): Variant;
var
  param: TClassProperty;
begin
  if Assigned(Obj) then begin
...
var
  ...
  current_instance: TPHPClassInstance;
  _instance_props: TStringList;
  prop_index: Integer;
begin
  Result:= nil;
  if _prop_value = nil then Exit;

  local_object_list:= _objects_list.LockList;
  try
    for instance_index := 0 to local_object_list.Count do begin
      current_instance:=  TPHPClassInstance(local_object_list[instance_index]);
      //si el objeto no tiene propiedades continuo con la proxima instancia.
      if current_instance.ObjectInternalProps = nil then Continue;
      _instance_props:= current_instance.ObjectInternalProps._propertyList;
      
      prop_index:= _instance_props.IndexOf(PropName);
      if (prop_index <> -1) then begin
        if Pointer(_instance_props.Objects[prop_index]) = _prop_value then begin
...
begin
  ...
  if local_instance <> nil then begin
    ...
    if index <> -1 then begin
      ...
      prop_index:= _instance_props.IndexOf(PropName);
      if prop_index <> -1 then begin
        Result:= Pointer(_instance_props.Objects[prop_index]);
      end;
    end;
    finally
      _objects_list.UnlockList;
...
begin
  ...
  if Assigned(Resource) then
   try
     object_list:=_objects_list.LockList;
     try
       //call the destroy instance.. Added by Luis E
       if Assigned(TPHPClassInstance(resource).Producer.OnDestroyInstance) then begin
         TPHPClassInstance(resource).Producer.OnDestroyInstance(resource);
       end;

       object_index := object_list.IndexOf(resource);
       if object_index <> -1 then
         object_list.Delete(object_index);
       resource.free;
     finally
       _objects_list.UnlockList;
     end;
...
begin
  ...
  if Assigned(Obj) then
   begin
    try
    _current_instance:= Obj;
    convert_to_string(value);
    param := Obj.Properties.ByName(propname);
    if Assigned(param) then
     param.Value := value.value.str.val;
    finally
     _current_instance:= _ant_instance;
    end
   end;
  Result := SUCCESS;
end;
    ...
    try
      ...
      if Assigned(param) then begin
        case param.DataType of
          ptString :
            begin
              variant2zval(param.value, retval);
            end;
          ptObject:
             begin
               if TryStrToInt(param.value, object_int) then
                 retval:= pzval(object_int)
               else
...
begin
  ...
  if Assigned(Obj) then begin
    try
      ...
      param := Obj.Properties.ByName(propname);
      if Assigned(param) then
        if value^._type <> IS_Object then begin
          param.Value := zval2variant(value^); //value.value.str.val;
        end  
        else begin
          //make a new zval to hold the reference (as an integer)....
          if (Param.ZendValue = nil) or (Param.ZendValue._type = IS_NULL)  then begin
            z:= MAKE_STD_ZVAL;
            ZVAL_LONG(z, Integer(value));
            param.Value := zval2variant(z^);
            //aumentar la referencia.
            Inc(z^.refcount);
          end
          else begin
            //decrementar la referencia.
            Dec(pzval(Integer(Param.value))^.refcount);
       ...
       begin
         if Assigned(M.FOnExecute) then
          begin
             ...
             try
               M.FOnExecute(Obj, M.Parameters, M.ReturnValue, M.FZendVar.AsZendVariable, this_ptr, TSRMLS_DC);
               if M.ZendVar.ISNull then begin  //perform variant conversion
                 variant2zval(M.ReturnValue, return_value);
               end;
             except
               zend_error(E_WARNING, PChar(Format('An exception occured while executing the %s.%s() delphi''s handler ',
                          [Producer.ClassName, get_active_function_name(TSRMLS_DC)])));
                               Result := FAILURE;
                               Exit;
             end;
          end;
   ...
   begin
      if (Extension.Components[cnt] is TphpClass) then
       begin
         if SameText(AClassName, TphpClass(Extension.Components[cnt]).PHPClassName) then
          begin
            TPHPClass(Extension.Components[cnt]).ProduceInstance(this_ptr, ht);
            break;
          end;
       end;
   end;
...
var
 ...
 begin
  ...
  if not Application.Loading and not Application.DeferredLoading then
   Exit;

  if (csDesigning in ComponentState) then
   Exit;

  if Application.Loading and  Application.DeferredLoading then begin
    Application.DeferClassRegistry(Self);
    Exit;
  end;
...
function GetParamsFromZvalArray(Params: pzval_array): Variant;
var
  ArrLen, i: Integer;
begin
  ArrLen := Length(Params);
  Result:= VarArrayCreate([0, ArrLen-1], varVariant);
  for i:=0 to ArrLen-1 do begin
    Result[i]:= zval2variant(Params[i]^^);
  end;
end;

function TPHPClass.ProduceInstance(AValue: pzval; ht: integer): TPHPClassInstance;
var
 CI : TphpClassInstance;
 rn : integer;
 tsrmls : pointer;
...
begin
 ...
 if not assigned(FClassObject) then begin
  Result:= nil;
  Exit;
 end;
 ...
 {$ENDIF}
  ...
  if Assigned (FOnInstanceCreate) then begin
    VarParams:= NULL;
    if ht > 0 then begin
      zend_get_parameters_ex(ht, Params);
      VarParams := GetParamsFromZvalArray(Params);
    end;
    set_current_instance(CI);
    try
    FOnInstanceCreate(CI, VarParams);
    finally
      unset_current_instance;
    end;
  end;
...
begin
  if (Source is TClassProperty) then
   begin
     ...
     FValue := TClassProperty(Source).FValue;
     FPropertyGetter:= TClassProperty(Source).OnGetValue;
     FPropertySetter:= TClassProperty(Source).OnSetValue ;
     FReadOnly:=TClassProperty(Source).ReadOnly;
     FPropertyType:= TClassProperty(Source).DataType;
   end
    else
     inherited;
end;

constructor TClassProperty.Create(Collection: TCollection);
begin
  inherited;
...
function TClassProperty.GetAsString: string;
var
  Component: TComponent;
begin
  if (Self.Collection.Owner is TComponent) then begin
    Component:= Self.Collection.Owner as TComponent;
    if not ((csDesigning  in Component.ComponentState) or (csLoading in Component.ComponentState)) then begin
...
var
  Value: Variant;
begin
  if Assigned(FPropertyGetter) then begin
    Value:= FPropertyGetter();
    Result:= Value;
  end
  else
...
function TClassProperty.GetZendValue: PZVal;
begin
  Result := FZendVariable.AsZendVariable;
end;
...
procedure TClassProperty.SetAsString(const Value: string);
var
  Component: TComponent;
begin
  if (Self.Collection.Owner is TComponent) then begin
...
procedure TClassProperty.SetValue(const Value: Variant);
var
  Component: TComponent;
begin
  if ReadOnly then Exit;
   if (Self.Collection.Owner is TComponent) then begin
    Component:= Self.Collection.Owner as TComponent;
    if ((csDesigning  in Component.ComponentState) or (csLoading in Component.ComponentState)) then begin
       SetAsString(Value);
       Exit;
    end;
   end;

  if Assigned(FPropertySetter) then begin
    FPropertySetter(Value);
  ...
  if (not Assigned(FZendVariable.AsZendVariable)) then
    FZendVariable.AsZendVariable:= MAKE_STD_ZVAL;
    
  FZendVariable.AsVariant := Value;
end;

procedure TClassProperty.SetZendValue(const Value: PZVal);
begin
...
destructor TClassProperties.Destroy;
begin
  inherited;
end;
...
constructor TObjectPropertyList.Create(_instance: TPHPClassInstance);
begin
  Self._instance:= _instance;
  Self._propertyList:= TStringList.Create;
end;

initialization
  _objects_list:= TThreadList.Create;
  _context_stack:= TStack.Create;
